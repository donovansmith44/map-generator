...
- **Every row carries its provenance — OWNER RULING, 2026-09-07 (R98).** "When we do the source condensation thing for phase 2, each piece of data has its own provenance (i.e. where we got the data from)." Every standing, every tenure, every law citation records **where the fact came from in the world**, not merely which file we typed it into. Those are two different things and this stage needs both: `source` (already in `Endurance::Endures`) names the authored file so a reader can find and audit the row; **provenance** names the origin — a scripture citation (book, chapter, verse), a named dataset and the row within it (`data/openbible/regions.geojson`, `data/wikimedia/tribes12.geojson`), or the atlas record it was derived from.

  **The condensation must carry the evidence with the claim, never separate them.** This plan as written splits them: `SurveySpec` today holds `book/chapter/verse_from/verse_to` *beside* the `Stands`/`Holds`/`Grade` those verses justify, and the retirement table above moves the claim into the ledger while leaving the citation behind in `surveys.rs` as "circuit evidence". A ledger row must cite what its survey row cited — otherwise the answer to "who says this people held this land?" stops being answerable from the answer itself, which is the whole point of a queryable ledger. Task 5 (`Stands` dies) and Task 6 (`Holds`/`Grade` die) are where this binds; Task 7's `stands_until` rows cite their geojson origin the same way.

  Provenance is a **first-class type, not a string** (owner's standing law applies to this too): a citation, a dataset row, a derivation from another entity, and an explicitly-declared *unknown* are distinct constructors. An unknown origin is representable ONLY as that declared variant with a written reason — never an empty string, never a default, never an omitted field. `dispose` carries provenance through to its output so `/api/disposition` can answer with it, and the census row schema stays frozen at five keys, so provenance rides the disposition route, not the census.
